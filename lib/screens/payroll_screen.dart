...
class _PayrollScreenState extends State<PayrollScreen> {
  ...
  Future<void> _downloadPayrollInvoice(Map<String, dynamic> data) async {
    ...
    try {
      ...
      // Attendance-based records (auto-generated by the payroll run review)
      // use the payable-days model: gross = daily rate × payable days and
      // deductions are already reflected in the payable day count.
      final attendanceBasedRecord = deductionsAreTotals;
      final dailyRateValue =
          (calculation['dailyRate'] as num?)?.toDouble() ?? 0;
      final overtimeValue = PayrollService.extractSalary(overtime);
      final proratedGross = dailyRateValue * workedDays;
      final totalDeductions = attendanceBasedRecord
          ? 0.0
          : PayrollService.extractSalary(
              calculation['formattedTotalDeductions'],
            );
      final netSalary = attendanceBasedRecord
          ? proratedGross + overtimeValue
      ...
      final bytes = await InvoiceService.generatePayrollInvoice(
        ...
        dailyRate: (calculation['formattedDailyRate'] ?? '${prefix}0.00')
            .toString(),
        grossPay: attendanceBasedRecord
            ? '$prefix${PayrollService.formatFullNumber(proratedGross)}'
            : (calculation['formattedGross'] ?? '${prefix}0.00').toString(),
        overtimePay: (calculation['formattedOvertime'] ?? '${prefix}0.00')
            .toString(),
        absentDeduction: attendanceBasedRecord
            ? '${prefix}0.00'
            : (calculation['formattedAbsentDeduct'] ?? '${prefix}0.00')
                  .toString(),
        leaveDeduction: attendanceBasedRecord
            ? '${prefix}0.00'
